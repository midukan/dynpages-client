...
import { AbstractControl, FormGroupDirective, NgModel, UntypedFormControl } from '@angular/forms';
...
export class BrMaskDirective implements OnInit {
  ...
  @HostListener('ngModelChange', ['$event']) onNgModelChange(e: any) {
    ...
    if (value) {
      this.setValueInFormControl(value, true);
    }
  }
  ...
  verifyFormControl(): boolean {
    if (this.brmasker.form instanceof UntypedFormControl) {
      return true;
    } else {
      return false;
    }
   ...
   */
  setValueInFormControl(value: string, viaNgModelChange = false): void {
